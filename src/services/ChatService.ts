...
const encrypt = encryptdecryptservice.encrypt;
const decrypt = encryptdecryptservice.decrypt;
...
export class ChatService {
    async createChat(data:  ChatData): Promise<string>{
        try{
            ...
            const character: ICharacter | null = await Character.findByPk(character_id);
            if (!character){
                logger.error("Character not found");
                return "AI character not found"
            };

            const chatHistory: IChat[] = await Chats.findAll({
            ...
            const messages = [
                { role: "system", content: `You are ${character.name}. Your personality: ${character.character_bio}` },
            ];

            chatHistory.reverse().forEach((chat: IChat) => {
                messages.push({ role: "user", content: decrypt(chat.user_message) });
                messages.push({ role: "assistant", content: decrypt(chat.character_response) });
            });
    ...
    async getChatHistory(user_id: number, character_id: number): Promise<IChat[]> {
        try {
            const chatHistory = await Chats.findAll({
                where: { user_id, character_id },
            });

            const response = chatHistory.map((it) => ({
                ...it.toJSON(),
